...
def get_message(key, **kwargs):
    if key == 'SERVER_WELCOME':
        return "Croeso i'r Gwasanaeth Negesu P2P"
    elif key == 'SERVER_CHOICE':
        return "1. Hostio gweinydd"
    elif key == 'SERVER_CONNECT':
        return "2. Cysylltu â gweinydd"
    elif key == 'SERVER_EXIT':
        return "3. Ymadael"
    elif key == 'SERVER_ENTER_CHOICE':
        return "Dewiswch eich dewis: "
    elif key == 'SERVER_HOSTING':
        return "[INFO] Gweinydd yn gwrando ar :12392"
    elif key == 'CLIENT_CONNECT':
        return "[INFO] {username} wedi ymuno o {ip}"
    elif key == 'CLIENT_DISCONNECT':
        return "[INFO] {username} wedi gadael y sgwrs."
    elif key == 'CLIENT_SEND_MESSAGE':
        return "[{username}] {message}"
    elif key == 'CLIENT_DUPLICATE_NAME':
        return "[INFO] Gwrthodwyd {username} oherwydd enw defnyddiwr dwbl"
    elif key == 'CLIENT_ERROR_RECEIVE':
        return "[ERROR] Gwall wrth dderbyn neges gan {username}: {error}"
    elif key == 'CLIENT_ERROR_HANDLE':
        return "[ERROR] Gwall wrth drin cleient {username}: {error}"
    elif key == 'CLIENT_ERROR_SEND':
        return "[ERROR] Methwyd anfon neges i'r cleient: {error}"
    elif key == 'CLIENT_SERVER_MESSAGE':
        return "[Gweinydd] {message}"
    elif key == 'CLIENT_ENTER_NAME':
        return "Rhowch eich enw: "
    elif key == 'CLIENT_NAME_TAKEN':
        return "[Gweinydd] Gwall: Enw defnyddiwr '{username}' eisoes wedi'i gymryd. Dewiswch enw gwahanol."
    return f"Mae allwedd neges '{key}' ddim wedi'i darganfod."
...
def handle_client(client_socket, client_address, client_name):
    ...
    try:
        ...
        broadcast_message(get_message('CLIENT_SERVER_MESSAGE', message=f"{client_name} wedi ymuno â'r sgwrs."))
        
        while True:
            # Receive a message from the client
            message = client_socket.recv(1024).decode('utf-8')
            if not message:
                print(f"[INFO] {client_name} wedi cawlio")
                break
        ...
        try:
            ...
            broadcast_message(get_message('CLIENT_SERVER_MESSAGE', message=f"{client_name} wedi gadael y sgwrs."))
        except ValueError:
...
def start_server():
    ...
    host = ''  # Gwrando ar bob rhyngwyneb ar gael
    port = 12392
...
def main():
    parser = argparse.ArgumentParser(description="Gwasanaeth Negeseu P2P")
    parser.add_argument('--host', action='store_true', help="Gosod gweinydd")
    ...
    if args.host:
        ...
        while True:
            ...
            if choice == '1':
                ...
                server_ip = input("Rhowch gyfeiriad IP y gweinydd: ")
                ...
                print("Dewis annilys. Rhowch 1, 2, neu 3.")
